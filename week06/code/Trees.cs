public static class Trees
{
    ...
    {
        // Stop recursive if index first or last is out of bounds
        if (first > last)
        {
            return;
        }
        else
        {
            ...
            // Add middle value
            bst.Insert(sortedNumbers[index]);

            // Insert values before the middle
            InsertMiddle(sortedNumbers, first, index - 1, bst);

            // Insert values after the middle
            InsertMiddle(sortedNumbers, index + 1, last, bst);
        }
    }
}
